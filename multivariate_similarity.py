...
def calculate_corr_dtw(target_features, compare_features):
    distances = []
    correlations = []
    for dim in range(target_features.shape[1]):
        dim_distance = dtw.distance_fast(target_features[:, dim], compare_features[:, dim])#* 0.25
...
def compute_distance(dt, users_target, users_compare, user_distance):
    ...
    for i in range(len(compare_data) - user_distance + 1):
        ...
        score = calculate_corr_dtw(target_values, compare_values)
...
def n_steps_ahead(data, values_list, options, n_steps=0, N=5):
    concatenated_df = pd.DataFrame()
    for column in options:
        changes = []
        target_data = data[column]
        for _, (_, end_date, _) in enumerate(values_list[:N], 1):
            sliced_data = target_data.loc[end_date:].iloc[:n_steps]
            sliced_data.reset_index(drop=True, inplace=True)
...
def main():
    ...
    if len(options) != 0:
        ...
        if st.sidebar.button('Generate'):
            ...
            original_figs = create_figures(original_data, [target_start_date, target_end_date], values_list, options, "(Original)", subtract=False, n_steps=nsteps, N = N)
            aligned_figs = create_figures(original_data, [target_start_date, target_end_date], values_list, options, "(Aligned)", subtract=True, n_steps=nsteps, N = N)
            ...
            if nsteps > 0:
                st.write("##### 유사기간 이후 변화")
                df = n_steps_ahead(original_data, values_list, options, n_steps= nsteps, N= N)
                st.table(df)
